...
export const ProfileCard = ({ profile, email }: ProfileCardProps) => {
  ...
  return (
    ...
    >
      <Card
        className="card-premium relative p-6 cursor-pointer group squircle transition-all duration-500 overflow-hidden shadow-2xl"
        onClick={() => navigate("/perfil")}
      >
        <div className="relative z-10 flex items-center gap-6">
          {/* Avatar Area */}
          <div className="relative">
            <Avatar className="w-20 h-20 squircle-sm border-2 border-border relative z-10 ring-4 ring-secondary/5">
            ...
            {/* Online Dot */}
            <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-green-500 rounded-full border-2 border-background z-20 shadow-lg shadow-green-500/20" />
          </div>
          ...
          <motion.div
            className="w-10 h-10 rounded-full bg-muted flex items-center justify-center border border-border"
            whileHover={{ scale: 1.1, backgroundColor: "hsl(var(--accent) / 0.1)" }}
          >
