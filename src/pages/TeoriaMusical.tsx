...
export default function TeoriaMusical() {
  ...
  return (
    <div className="w-full max-w-full overflow-x-hidden min-h-screen bg-[#0a0e17] text-slate-100 p-3 sm:p-6 pb-28 space-y-5">
      {/* Mobile-Friendly Friendly Banner */}
      <motion.div
        initial={{ opacity: 0, y: -10 }}
        animate={{ opacity: 1, y: 0 }}
        className="relative rounded-2xl sm:rounded-3xl p-4 sm:p-6 overflow-hidden bg-gradient-to-r from-purple-950/90 via-slate-900/95 to-indigo-950/90 border border-purple-500/20 shadow-xl backdrop-blur-2xl"
      >
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="space-y-1.5 min-w-0">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-purple-500/20 text-purple-300 border border-purple-500/30 text-[11px] font-bold uppercase tracking-wider">
              <GraduationCap className="w-3.5 h-3.5" /> Escuela de Música
            </div>
            <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight">
              Academia <span className="text-purple-400">Musical</span>
            </h1>
            <p className="text-slate-300 text-xs sm:text-sm leading-relaxed">
              Videos, guías PDF y técnica para músicos y voces de todas las edades.
            </p>
          </div>
          ...
          {canManageTheory && (
            <Button
              ...
              className="w-full sm:w-auto h-12 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold px-5 shadow-lg shadow-purple-600/30 text-sm shrink-0 flex items-center justify-center gap-2"
            >
              <Plus className="w-5 h-5" /> Publicar Recurso
            </Button>
          )}
        </div>
      </motion.div>

      {/* Touch-Friendly Search & Favorite Tab Bar */}
      <div className="space-y-3">
        ...
        {/* Scrollable Instrument Pills (Touch-Optimized) */}
      ...
      {/* Difficulty Level Selector (Simple Row) */}
      ...
      {/* Resource Cards (Mobile-First 1 Column) */}
      {resourcesLoading ? (
